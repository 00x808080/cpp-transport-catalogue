...
// Структура для вывода Color через visit
struct ColorPrinter {
    std::ostream& out;
    ...
    void operator()(const Rgb& rgb) const {
        out << "rgb(" << unsigned(rgb.red) << ','
                      << unsigned(rgb.green) << ','
                      << unsigned(rgb.blue) << ')';
    }
    void operator()(const Rgba& rgba) const {
        out << "rgba(" << unsigned(rgba.red) << ','
                      << unsigned(rgba.green) << ','
                      << unsigned(rgba.blue) << ','
                      << rgba.opacity << ')';
    }
...
private:
    // Метод для работы method chaining
    Owner& AsOwner() {
...
// Интерфейс Drawable унифицирует работу с объектами,
// которые можно нарисовать, подключив SVG-библиотеку.
class Drawable {
public:
